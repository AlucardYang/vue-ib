<template>
  <div class="com-frame" style="padding-bottom:.5rem;" :class="{'com-wxix-frame': (wxixm || fbixm)}">  
    ...
    <button class="com-btn com-button-action aux-abs-full" :class="{'com-wxix-button': (wxixm || fbixm)}" type="button" @click="enterNextPage">
      <label v-if="simply">咨询顾问</label>
      <label v-else>諮詢顧問</label>
    </button> 
...
<script>
  ...
  export default {
    ...
    created () {
      ...
      this.v2_c_user_uuid = query.v2_c_user_uuid;
