...
export default {
  ...
  methods: {
    ...
    goAgentURL: function() {
      window.location.href = this.info.iber_info.home_page_url;
    },
    call: function() {
  ...
  beforeRouteLeave (to, from, next) { 
    if (this.comAgentWechatData.layer > 0) { 
      this.comAgentWechatData.layer -= 1;
      next(false);    
      return ; 
    }    

    let callbackURL = plug.jsonToUrl().callback_url;
    if (plug.valueValid(callbackURL)) { 
      console.log("返回");
  ...
  created () {  
    ...
    if (plug.valueValid(query) && plug.valueValid(query.user_uuid) && plug.valueValid(query.answer_uuid) && plug.valueValid(query.from_type)) {
      ...
      this.v2_c_user_uuid = this.$route.query.v2_c_user_uuid;
    } else {
      ...
      this.v2_c_user_uuid = this.urlParams.v2_c_user_uuid;   
    } 
    
    //重置登录态-peter提供
    if (plug.valueValid(this.v2_c_user_uuid) && localStorage.getItem('IBER_BS_USER_INFO-' + this.userUuid)) {
      let userInfo = JSON.parse(localStorage.getItem('IBER_BS_USER_INFO-' + this.userUuid)).data;
      userInfo.c_user_uuid = this.v2_c_user_uuid;
    ...
    plug.authAddress({
      ...
      v2_c_user_uuid: this.v2_c_user_uuid,
      from_ib_url: query.from_ib_url,
