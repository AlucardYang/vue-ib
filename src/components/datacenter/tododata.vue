<template>
    <div id="todoData">
        <div v-if="loadedScript && isLoaded" class="todo-data-inner com-scroll-y">
            ...
            <data-center-line-chart :type="selectType" :tabs="tabs" :unit="'次'" :title="simply ? '行动量增长趋势' : '行動量增長趨勢'" :y-data1="viewData.todo_trend" :show-total="trendTotal"></data-center-line-chart>
            <data-center-flower-pie-chart :title="this.simply ? '行动量类型' : '行動量類型'" :data="viewData.todo_cate" :show-num="true" :other-legend="viewData.todo_cate_others"></data-center-flower-pie-chart>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        getTodoData() {
            ...
            this.$http.get('/data-center/todo-data', {
                ...
                if (res && res.code === 0) {
                    ...
                    this.trendTotal = {
                        show: true,
                        title: this.simply ? '历史行动量总数：' : '歷史行動量總數：',
                        num: this.viewData.todo_total
                    };
                }
                this.isLoaded = true;
