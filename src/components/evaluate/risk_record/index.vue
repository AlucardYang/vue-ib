    ...
    <div class="com-scroll-y aux-bar com-bg aux-pdm">
      <header style="padding:.12rem;">
        <div class="iber-card">
            ...
            <div class="info">
              <div class="com-disx">
                ...
                <div> 
                  ...
                  <p class="com-wrap" v-if="plug.valueValid(record.mobile)">{{plug.sm(simply, '授權手機')}}: {{record.mobile}}
                  </p>
                </div> 
              </div>
            </div>
          ...
          <p class="com-nowrap com-disx show">
            <span>{{record.create_at}}{{plug.sm(simply, '完成評測')}}</span>
            <em @click="enterPage('result')">{{plug.sm(simply, '查看結果')}}<i class="iconfont">&#xe6a5;</i></em>
          </p>
          <p class="note" v-if="plug.valueValid(msgID)" @click="consultRecord">
            <i class="iconfont">&#xe650;</i>{{plug.sm(simply, '有諮詢紀錄')}}
          </p>
        </div>
      </header>
      <ul>
      ...
      <dl class="dialog agent" v-if="state">
        ...
        <dd class="result">
          <div>
            <div>
              <h3>{{plug.sm(simply, '智能風險評測結果')}}</h3>
              <p>{{plug.sm(simply, 'TiMi已經思考完畢了~為你寫了一份個人風險評測報告噢！')}}</p>
            </div>
...
export default {
  ...
  methods: {
    ...
    enterPage: function(type) {
      ...
      if (type == 'result') {
        cmn.czcEv('bdj_fengxianchakanjieguo', '查看结果');
      }
      if (type == 'detail') {
        cmn.czcEv('bdj_fengxianchakanxiangqing', '查看详情');
      }
      this.$router.push({
        path: "/evaluate/risk-app",
        query: {
  ...
  created() {
    document.title = this.simply ? "风险评测记录" : "風險評測紀錄";
